#
#
####################################### DA QUI PRENDIAMO IL GENERATORE #########################################
#
#
...
class FullyConnectedLayer(torch.nn.Module):
    ...
    def forward(self, x):
        w = self.weight.to(x.dtype) * self.weight_gain #era x
        b = self.bias
        if b is not None:
            b = b.to(x.dtype)
            if self.bias_gain != 1:
        ...
        if self.activation == 'linear' and b is not None:
            x = torch.addmm(b.unsqueeze(0), x[0,:,:,:], w.t().unsqueeze(1))
        else:
            x = x.matmul(w.t())
...
class SynthesisInput(torch.nn.Module):
    def __init__(self,
                 img_real_dim,  # Intermediate latent (W) dimensionality.
        ...
        super().__init__()
        self.img_real_dim = img_real_dim
        ...
        self.affine = FullyConnectedLayer(img_real_dim, 4, weight_init=0, bias_init=[1, 0, 0, 0])
    ...
    def extra_repr(self):
        return '\n'.join([
            f'img_real_dim={self.img_real_dim:d}, channels={self.channels:d}, size={list(self.size)},',
...
class SynthesisLayer(torch.nn.Module):
    def __init__(self,
        ...
        # Setup parameters and buffers.
        self.affine = FullyConnectedLayer(self.img_real_dim, self.in_channels, bias_init=1)
    ...
    def forward(self, x, w, noise_mode='random', force_fp32=False, update_emas=False):
        ...
        misc.assert_shape(w, [x.shape[0], self.img_real_dim])
    ...
    def extra_repr(self):
        return '\n'.join([
            f'img_real_dim={self.img_real_dim:d}, is_torgb={self.is_torgb},',
...
class SynthesisNetwork(torch.nn.Module):
    def __init__(self,
                 img_real_dim,  # Real image dimensionality.
        ...
        self.input = SynthesisInput(
            img_real_dim=self.img_real_dim, channels=int(channels[0]), size=int(sizes[0]),
        ...
        for idx in range(self.num_layers + 1):
            ...
            layer = SynthesisLayer(
                img_real_dim=self.img_real_dim, is_torgb=is_torgb, is_critically_sampled=is_critically_sampled,
                use_fp16=use_fp16,
    ...
    def forward(self, real_img, **layer_kwargs):
        # misc.assert_shape(real_img, [None, self.num_ws, self.img_real_dim])
        #ws = real_img.to(torch.float32).unbind(dim=1)
        ws = real_img.to(torch.float32)

        # Execute layers.
        x = self.input(ws) # era ws[0]
    ...
    def extra_repr(self):
        return '\n'.join([
            f'img_real_dim={self.img_real_dim:d}, num_ws={self.num_ws:d},',
...
class Generator(torch.nn.Module):
    def __init__(self,
        ...
        self.img_resolution = img_resolution
        self.img_channels = img_channels
        self.synthesis = SynthesisNetwork(img_real_dim=img_real_dim, img_resolution=img_resolution,
                                          img_channels=img_channels, **synthesis_kwargs)

    def forward(self, real_img, update_emas=False, **synthesis_kwargs):
        img = self.synthesis(real_img, update_emas=update_emas, **synthesis_kwargs)
        im = copy.copy(img)
